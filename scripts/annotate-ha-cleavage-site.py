...
parser.add_argument(
    "--alignment", type=str, help="alignment file output by rule augur align"
)
parser.add_argument(
    "--furin_site_motif",
    type=str,
    help="name of output json file that annotates tips as having a furin cleavage site or a wt cleavage site",
)
parser.add_argument(
    "--cleavage_site_sequence",
    type=str,
    help="name of output json file that annotates the cleavage site sequence for each tip",
)
...
def output_furin_cleavage_site_jsons(alignment, output_json1, output_json2):
    ...
    start_pos = find_universal_start(alignment)
    ...
    for seq in SeqIO.parse(alignment, "fasta"):
        ...
        nt_sequence = str(seq.seq)[start_pos:].upper().replace("-", "N")
        ...
        if start_pos_ha2_nt > 0:
            ...
            if (
                furin_site[0] == "R"
                and furin_site[3] == "R"
                and (furin_site[2] == "R" or furin_site[2] == "K")
                and furin_site[1] != "X"
            ):
        ...
        output_dict_furin["nodes"][strain_name] = {
            "furin_cleavage_motif": furin_site_annotation
        }
        output_dict_seq["nodes"][strain_name] = {
            "cleavage_site_sequence": furin_site.replace("X", "-")
        }
...
output_furin_cleavage_site_jsons(
    alignment, furin_site_motif_json, cleavage_site_sequence_json
)
